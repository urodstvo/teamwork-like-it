...
export function renderSelectionFrame(ctx: CanvasRenderingContext2D, selectionFrame: Exclude<EditorState['selectionFrame'], null>, canvasOffset: EditorState['canvasOffset']) {
	ctx.save()
	ctx.strokeStyle = '#00acfd'
	ctx.fillStyle = '#00acfd'

	const xWithOffset = selectionFrame.leftX + canvasOffset.offsetX
	const yWithOffset = selectionFrame.leftY + canvasOffset.offsetY
